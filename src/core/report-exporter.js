...
class ReportExporter {
     ...
     */
    async exportScanResults(results, tokenReport, options) {
     ...
     */
    async exportHTML(results, tokenReport, outputPath) {
     ...
     */
    async exportPDF(results, tokenReport, outputPath) {
...
`;
     ...
     */
    async exportMarkdown(results, tokenReport, outputPath) {
     ...
     */
    generateHTML(results, tokenReport) {
        const avgScore = results.length > 0
            ? Math.round(results.reduce((sum, r) => sum + r.score, 0) / results.length)
            : 0;
        const criticalCount = results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'critical').length, 0);
        const highCount = results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'high').length, 0);
        const mediumCount = results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'medium').length, 0);
        const lowCount = results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'low').length, 0);
        return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="container">
     ...
     */
    generateMarkdown(results, tokenReport) {
        const avgScore = results.length > 0
            ? Math.round(results.reduce((sum, r) => sum + r.score, 0) / results.length)
            : 0;
        const criticalCount = results.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'critical').length, 0);
